...
//#define SAMPLE_EXHAUSTIVELY
        ...
        Context.SetDynamicDescriptor(0, DepthBuffer.GetSRV() );
...
void SSAO::Render( GraphicsContext& GfxContext, const float* ProjMat, float NearClipDist, float FarClipDist )
{
    ...
    if (!Enable)
    {
        ...
        Context.SetDynamicDescriptor(0, LinearDepth.GetUAV());
    ...
    ComputeContext& Context = GfxContext.GetComputeContext();
    Context.SetRootSignature(s_RootSignature);
    D3D11_SAMPLER_HANDLE Sampler[] = { SamplerLinearClamp, SamplerLinearBorder };
    Context.SetDynamicSamplers( 0, 2, Sampler );
    ...
    D3D11_UAV_HANDLE DownsizeUAVs[5] = { LinearDepth.GetUAV(), g_DepthDownsize1.GetUAV(), g_DepthTiled1.GetUAV(),
        g_DepthDownsize2.GetUAV(), g_DepthTiled2.GetUAV() };
    Context.SetDynamicDescriptors(0, 5, DownsizeUAVs);
    ...
    if (HierarchyDepth > 2)
    {
        ...
        D3D11_UAV_HANDLE DownsizeAgainUAVs[4] = { g_DepthDownsize3.GetUAV(), g_DepthTiled3.GetUAV(), g_DepthDownsize4.GetUAV(), g_DepthTiled4.GetUAV() };
        Context.SetDynamicDescriptors(0, 4, DownsizeAgainUAVs);
        Context.SetDynamicDescriptor(0, g_DepthDownsize2.GetSRV() );
    ...
    if (DebugDraw)
    {
        ...
        CC.SetDynamicDescriptor(0, g_SceneColorBuffer.GetUAV());
        CC.SetDynamicDescriptor(0, g_SSAOFullScreen.GetSRV());
